...
prob2thresh <- function(x, prob, ref.ind = NULL, multi.model = FALSE) {
  ...
  if (length(unique(ref.ind)) <= 1) { ## condition for both is.null or all equal
    ...
    if (multi.model) {
      ...
      threshold <- array(
        threshold[rep(1:nrow(threshold), length = length(prob) * nrow(as.matrix(x))), ],
        c(length(prob), size(x))
      )
    } else {
      threshold <- array(
        quantile(as.matrix(xthresh)[ref.ind[[1]], ], sort(prob), na.rm = T, type = 8),
        c(length(prob), size(x))
      )
    }
  } else {
...
expandthresh <- function(threshold, x) {
  ...
  if (is.vector(threshold)) {
    ...
    threshold <- array(
      threshold[, rep(1:ncol(threshold), each = nrow(x))],
